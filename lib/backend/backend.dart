...
Future<int> queryLabelsRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
...
Future maybeCreateUser(User user) async {
  final userRecord = AccountsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await AccountsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createAccountsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      AccountsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createAccountsRecordData(email: email));
}
